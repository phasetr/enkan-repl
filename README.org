...
*** Workspace Support (Experimental)
- All sessions are now scoped to a single workspace (ID: =01=)
- Buffer naming format: =*ws:01 enkan:/path/to/directory*=
- Session operations (setup/teardown) are workspace-isolated
- Future versions will support multiple concurrent workspaces
