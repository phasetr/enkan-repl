...
- =enkan-repl-send-4= - Send \\='4\\=' to enkan session buffer for numbered choice prompt. - From enkan buffer: Send to current buffer - From other buffer without prefix: Interactive buffer selection - With numeric prefix: Send to buffer at index (1-based)  Uses unified backend with smart buffer detection.
- =enkan-repl-send-5= - Send \\='5\\=' to enkan session buffer for numbered choice prompt. - From enkan buffer: Send to current buffer - From other buffer without prefix: Interactive buffer selection - With numeric prefix: Send to buffer at index (1-based)  Uses unified backend with smart buffer detection.
- =enkan-repl-send-escape= - Send ESC key to enkan session buffer. - From enkan buffer: Send to current buffer - From other buffer without prefix: Interactive buffer selection - With numeric prefix: Send to buffer at index (1-based)  Uses unified backend with smart buffer detection.
...
- =enkan-repl-teardown= - Terminate eat session(s) based on context. - Standard input file: terminate single eat session for current directory - Center file: terminate all registered sessions
