...
- Each project alias becomes a tmux window in that session.  Multiple
  ...
  inside Emacs.
- A read-only mirror buffer is updated periodically via
  =tmux capture-pane= so the layout / send-line code can find sessions
  via the usual =buffer-list= mechanism.  Disable mirroring with
  =(setq enkan-repl-tmux-mirror nil)= if not desired.
