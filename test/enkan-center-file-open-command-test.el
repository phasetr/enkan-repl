...
(defun enkan-center-file-validate-path (file-path)
...
(defun enkan-center-file-check-exists (file-path)
...
(defun enkan-center-file-determine-action (file-path)
  "Determine action to take for center FILE-PATH.
Returns plist with :valid, :action, :message."
  (let ((validation (enkan-center-file-validate-path file-path)))
    (if (plist-get validation :valid)
        (let ((exists-check (enkan-center-file-check-exists file-path)))
          (list :valid t
...
(ert-deftest test-enkan-center-file-validate-path ()
  ...
  (should-not (plist-get (enkan-center-file-validate-path nil) :valid))
  (should-not (plist-get (enkan-center-file-validate-path 123) :valid))
  (should-not (plist-get (enkan-center-file-validate-path "") :valid))
  (should (plist-get (enkan-center-file-validate-path "/valid/path.org") :valid)))
...
(ert-deftest test-enkan-center-file-exists-check ()
  ...
  (let ((temp-file (make-temp-file "center-test" nil ".org")))
    (unwind-protect
        (progn
          (should (plist-get (enkan-center-file-check-exists temp-file) :exists))
          (should (equal (plist-get (enkan-center-file-check-exists temp-file) :action) "open")))
      (when (file-exists-p temp-file)
        (delete-file temp-file))))
  
  (should-not (plist-get (enkan-center-file-check-exists "/nonexistent/file.org") :exists))
  (should (equal (plist-get (enkan-center-file-check-exists "/nonexistent/file.org") :action) "create")))

;; Test action determination
(ert-deftest test-enkan-center-file-determine-action ()
  "Test center file action determination."
  ;; Invalid paths
  (should-not (plist-get (enkan-center-file-determine-action nil) :valid))
  (should-not (plist-get (enkan-center-file-determine-action "") :valid))
  
  ;; Valid path for existing file
  (let ((temp-file (make-temp-file "center-test" nil ".org")))
    (unwind-protect
        (let ((result (enkan-center-file-determine-action temp-file)))
  ...
  (let ((result (enkan-center-file-determine-action "/nonexistent/test.org")))
    (should (plist-get result :valid))
